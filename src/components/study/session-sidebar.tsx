...
import { ChevronDown, FileText, FolderOpen, FolderClosed, BookText, ChevronsLeftRight, PlusSquare, FilePlus2, FolderPlus, Dot } from 'lucide-react';
...
export interface TreeNode {
  ...
  children?: TreeNode[]; // Undefined for 'note' type
...
const TreeItemDisplay: React.FC<TreeItemDisplayProps> = ({ item, level, onSelectNode, activeNodeId, onAddNode }) => {
  ...
  if ((item.type === 'title' || item.type === 'subheading') && item.children) { // Check item.children specifically for folder types
  ...
  const handleToggleOpen = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (item.type !== 'note' && item.children) { // Only non-notes with children can be opened/closed
      setIsOpen(!isOpen);
    }
  };
  ...
  const canAddNote = item.type === 'subject' || item.type === 'title' || item.type === 'subheading';
  ...
  return (
    <div>
      ...
      >
        {item.type !== 'note' && item.children ? ( // Chevron for items that can have children
          <button onClick={handleToggleOpen} className="p-0.5 mr-1.5 rounded-sm hover:bg-white/20 focus:outline-none shrink-0">
           ...
           item.type !== 'subject' && <span className="w-4 mr-1.5 shrink-0 flex items-center justify-center"><Dot className="h-4 w-4 opacity-50"/></span>
        )}
      ...
      {/* Render Input for Adding New Child */}
      {isOpen && editingChild && (
        <div 
          className="flex items-center py-1.5 pr-1 ml-[calc(0.5rem_+_0.8rem)]" // Matches child indentation logic
          style={{ paddingLeft: `${(level + 1) * 0.0}rem` }} // Additional indent for input within child list
        >
          ...
          <form onSubmit={handleConfirmAddChild} className="flex-1">
            <input
              ...
              onBlur={(e) => {
                // Check if the blur was to an element inside this form or related controls
                // For simplicity, confirm if name is present, otherwise cancel.
                 if (editingChild && editingChild.name.trim()) {
  ...
  if (isCollapsed) {
    return (
      <div className="w-[60px] h-full bg-[#0F0F0F] text-white p-3 flex flex-col items-center space-y-4 shrink-0 transition-all duration-300 ease-in-out border-r border-white/10">
        ...
        {rootNode && <BookText className="h-6 w-6 text-primary cursor-pointer hover:opacity-80" title={sessionSubject || "Notebook"} onClick={() => onSelectNode(rootNode.id, 'subject')}/>}
        {/* Simplified icons for collapsed mode - adding needs context, so maybe remove add button here */}
        {/* <PlusSquare className="h-6 w-6 text-foreground-opacity-70 cursor-pointer hover:opacity-80" title="Add Note" onClick={() => onAddNode(rootNode?.id || null, 'note', prompt("Enter note name:") || "New Note")}/> */}
      </div>
    );
  }
    ...
    <div className="w-[280px] h-full bg-[#0F0F0F] text-white p-3 flex flex-col shrink-0 overflow-y-hidden custom-scrollbar transition-all duration-300 ease-in-out border-r border-white/10">
      <div className="flex items-center justify-between mb-3 pb-2 border-b border-white/10">
        {rootNode ? (
            <div 
                className={cn(
                    "flex items-center gap-2 w-full mr-1 cursor-pointer rounded-md p-1",
            ...
            >
             ...
             <div className="flex items-center gap-2 w-full mr-1 p-1">
                <BookText className="h-5 w-5 text-primary flex-shrink-0" />
      ...
      <ScrollArea className="flex-1 -ml-2 pr-1"> 
         {rootNode && rootNode.children && rootNode.children.map(childNode => (
             <TreeItemDisplay 
                ...
                onSelectNode={onSelectNode} 
                activeNodeId={activeNodeId}
                onAddNode={(parentId, type, name) => onAddNode(parentId, type, name)} // Ensure correct prop mapping
              />
         ))}
         {rootNode && (!rootNode.children || rootNode.children.length === 0) && ( // Check if root exists before this
            <p className="text-xs text-muted-foreground p-2 text-center">No items yet. Hover over "{sessionSubject}" to add.</p>
         )}
         {!rootNode && (
